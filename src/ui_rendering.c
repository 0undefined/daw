...
void render_container(Window* w, UITree_container* t) {
  SDL_Rect r = {.x = t->x, .y = t->y, .w = t->w, .h = t->h};

  SDL_SetRenderDrawColor(w->renderer, t->bg.r, t->bg.g, t->bg.b, t->bg.a);

  SDL_RenderFillRect(w->renderer, &r);

  SDL_SetRenderDrawColor(w->renderer, t->fg.r, t->fg.g, t->fg.b, t->fg.a);
  ...
  r.w -= t->padding * 2;
  r.h -= t->padding * 2;
  SDL_SetRenderDrawColor(w->renderer, 0xFF, 0xFF, 0xFF, 0x7A);
  SDL_RenderDrawRect(w->renderer, &r);
#endif
...
void render_button(Window* w, UITree_button* t) {
  ...
  SDL_SetRenderDrawColor(w->renderer, fg.r, fg.g, fg.b, fg.a);

  SDL_RenderDrawRect(w->renderer, &outer_r);
  SDL_Texture* texture = ((struct Resources*)GLOBAL_PLATFORM->data)
                             ->textures[t->text_texture_index]
                             ->texture;
  SDL_RenderCopy(w->renderer, texture, NULL, &inner_r);
}
...
void render_title(Window* w, UITree_title* t) {
  ...
  SDL_RenderCopy(w->renderer, texture, NULL, &r);
...
i64 add_texture(struct Resources* resptr, Texture* t) {
  if (NULL == resptr || NULL == t) return -1;
  ...
  if (resptr->textures == NULL) {
    ...
    resptr->textures =
        realloc(resptr->textures, sizeof(Texture*) * resptr->textures_size);
    memset(resptr->textures + resptr->textures_size - TEXTURES_INCREMENT, 0,
           TEXTURES_INCREMENT);
  }
...
i64 engine_render_text(i32 font_id, Engine_color fg, char* text,
  ...
  if (wrapped) {
    s = TTF_RenderUTF8_Solid_Wrapped(r->fonts[font_id], text, sdl_fg,
                                     size_out->x);
  ...
  t->texture =
      SDL_CreateTextureFromSurface(GLOBAL_PLATFORM->window->renderer, s);
